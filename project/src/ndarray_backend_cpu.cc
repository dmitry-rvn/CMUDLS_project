    ...
    */
    ...
    for (size_t i = 0; i < m / TILE; i++) {
        for (size_t j = 0; j < p / TILE; j++) {
            for (size_t h = 0; h < TILE * TILE; h++)
                tmp[h] = 0;
            for (size_t k = 0; k < n / TILE; k++) {
                for (size_t h = 0; h < TILE * TILE; h++) {
                  size_t row = h / TILE, col = h % TILE;
                  A[h] = a.ptr[i * TILE * n + k * TILE * TILE + h];
                  B[h] = b.ptr[k * TILE * p + j * TILE * TILE + h];
                }
                AlignedDot(A, B, tmp);
            }
            for (size_t h = 0; h < TILE * TILE; h++)
                out->ptr[i * p * TILE + j * TILE * TILE + h] = tmp[h];
        }
    }
}
