...
def compute_gradient_of_variables(output_tensor: Tensor, out_grad: Tensor):
    ...
    for node in reverse_topo_order:
        ...
        if node.inputs:
            ...
            for child_node, child_grad in zip(node.inputs, child_grads):
                node_to_output_grads_list[child_node].append(child_grad)
...
def find_topo_sort(node_list: list[Value]) -> list[Value]:
    ...
    topo_order = topo_sort_dfs(node_list[0], set(), [])
    return topo_order


def topo_sort_dfs(node: Value, visited: set[Value], topo_order: list[Value]) -> list[Value]:
    """Post-order DFS"""
