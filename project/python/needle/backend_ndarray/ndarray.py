...
class NDArray:
    ...
    def as_strided(self, shape, strides):
        ...
        assert len(shape) == len(strides), f'{len(shape)} != {len(strides)}'
        return NDArray.make(
    ...
    def reshape(self, new_shape):
        ...
        #     raise ValueError(f'Product of current shape {self.shape} is not equal to product of new shape {new_shape}')
        return self.compact().as_strided(shape=new_shape, strides=NDArray.compact_strides(new_shape))
    ...
    def __getitem__(self, idxs):
        ...
        new_offset, new_shape, new_strides = 0, list(self.shape), list(self.strides)
        ...
        return NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides), device=self.device,
    ...
    def __matmul__(self, other):
        ...
        assert self.ndim == 2 and other.ndim == 2, f'self.ndim={self.ndim}, other.ndim={other.ndim}'
    ...
    def flip(self, axes):
        ...
        offset, new_strides = 0, list(self.strides)
        for axis in axes:
            offset += self.strides[axis] * (self.shape[axis] - 1)
    ...
    def pad(self, axes):
        ...
        a = NDArray.make(tuple(new_shape), device=self.device)
        a.fill(0)
        slices = tuple([slice(axis[0], axis[0] + self.shape[i]) for i, axis in enumerate(axes)])
        a[slices] = self
        return a
